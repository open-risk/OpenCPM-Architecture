...
class TabularCellRange(CellRange):
    ...
    def __getitem__(self, key):
        ...
        if not isinstance(key, tuple) and not isinstance(key, str):
            ...
            range=self.document._target.NamedRanges.getByName(key).ReferredCells.RangeAddress.value
            row, row_count=address.row +range.StartRow, range.EndRow-range.StartRow+1
            col, col_count=address.col +range.StartColumn, range.EndColumn-range.StartColumn+1
        if range.EndRow-range.StartRow == 1:
            single_row=True
            single_col=False
        elif range.EndColumn-range.EndColumn == 1:
            single_row=False
            single_col=True
        elif range.EndRow-range.StartRow == 1 & range.EndRow-range.StartRow == 1 :
            single_row=True
            single_col=True
        else:
            single_row=False
